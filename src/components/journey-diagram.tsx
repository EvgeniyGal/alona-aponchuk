...
const stepIcons = [UserPlus, ClipboardList, Stethoscope, CalendarDays, HeartHandshake, MessageSquare, Repeat];

export function JourneyDiagram() {
  const t = useTranslations("home.journey");
  const steps = t.raw("steps") as Array<{ label: string; meta: string }>;
    ...
    <div className="relative z-10 rounded-2xl border border-hairline bg-white p-6 shadow-[0_1px_0_rgba(31,41,51,0.03),0_20px_40px_-24px_rgba(31,41,51,0.15)]">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <div className="mt-1 font-display text-[15px] font-semibold text-graphite">{t("title")}</div>
        </div>
        <div className="hidden sm:flex items-center gap-2 text-[11px] text-muted-foreground">
          <span className="status-ping inline-block h-2 w-2 rounded-full bg-sage text-sage" /> {t("onTrack")}
          <span className="ml-3 inline-block h-2 w-2 rounded-full bg-gold" /> {t("needsReview")}
        </div>
      </div>
            ...
            >
              ...
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10.5px] font-medium ${
                  status === "gold" ? "bg-[#f6ecd7] text-[#8a6d2b]" : "bg-[#e5ede5] text-[#3b5a3f]"
                }`}
              >
      ...
      <div className="mt-5 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-md bg-teal-soft py-2 text-[11px] font-medium text-graphite">{t("crm")}</div>
        <div className="rounded-md bg-blue-soft py-2 text-[11px] font-medium text-graphite">{t("scheduling")}</div>
        <div className="rounded-md bg-[#e5ede5] py-2 text-[11px] font-medium text-graphite">{t("messaging")}</div>
      </div>
    </div>
  );
}
